...
def filename2(n,file, res):return '/home/blair/Documents/Mordor/'+file+'_Cph/'+file+'_'+str(res)+'_di_'+str(n)+'.dat'
def filename3(n,file, res):return '/home/blair/Documents/Mordor/'+file+'_Cph/'+file+'_'+str(res)+'_di_'+str(n)+'.dat'
...
for file in filearray:
    #Loop through pH
    levels, f_max = create_levels(file)
    for n in range(0,10,1):
        #Load Data
        hisAng = np.loadtxt(filename4(n,file))
        hisDiH = np.loadtxt(filename2(n,file, Residue[res]))
        ...
        plt.savefig('/home/blair/Documents/temp/' + file + '_3D_angle_contour_pH' + str(n + 1) + '_longer.png',bbox_inches='tight')
        # plt.show()
        plt.close()
    res += 2